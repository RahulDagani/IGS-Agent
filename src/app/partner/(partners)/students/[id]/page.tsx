...
export default function StudentDetailsPage() {
  ...
  useEffect(() => { fetchStudentDetails(); }, []);
  useEffect(() => { if (activeTabFromUrl) setActiveTab(activeTabFromUrl); }, [activeTabFromUrl]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-6 gap-6">
        {/* Student Info Card */}
        <div className="col-span-2 rounded-lg border border-gray-300 dark:border-gray-700 p-4">
          <div className="space-y-3">
            <div>
              <span className="font-semibold text-lg dark:text-white">{student.name}</span>
            </div>
        ...
        {/* Main Tabs */}
        <div className="col-span-4 rounded-lg border border-gray-300 dark:border-gray-700 p-4">
          <ul className="flex flex-wrap justify-around" role="tablist">
            <li className="mr-2">
              <button
                className={`flex flex-col items-center px-4 py-3 font-medium text-sm ${activeTab === "profile" ? "text-blue-600 dark:text-blue-400" : "text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300"}`}
                onClick={() => setActiveTab("profile")}
              >
                <span className="mb-2 bg-gray-200 dark:bg-gray-700 rounded-full w-6 h-6 flex items-center justify-center text-xs">1</span>
                <span className="flex justify-center items-center gap-1">
                  <span>Profile</span>
                  {student.profile_status === "COMPLETE"
                    ? <span className="bg-green-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-xs">✓</span>
                    : <span className="bg-red-700 text-white rounded-full w-4 h-4 flex items-center justify-center text-xs">✗</span>}
                </span>
              </button>
            </li>
            <li className="mr-2">
              <button
                className={`flex flex-col items-center px-4 py-3 font-medium text-sm ${activeTab === "applications" ? "text-blue-600 dark:text-blue-400" : "text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300"}`}
                onClick={() => setActiveTab("applications")}
              >
                <span className="mb-2 bg-gray-200 dark:bg-gray-700 rounded-full w-6 h-6 flex items-center justify-center text-xs">2</span>
                <span className="flex justify-center items-center gap-1">
                  <span>Applications</span>
                  <span className="bg-green-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-xs">{student.total_applications}</span>
                </span>
              </button>
            </li>
            <li>
              <button
                className={`flex flex-col items-center px-4 py-3 font-medium text-sm ${activeTab === "documents" ? "text-blue-600 dark:text-blue-400" : "text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300"}`}
                onClick={() => setActiveTab("documents")}
              >
                <span className="mb-2 bg-gray-200 dark:bg-gray-700 rounded-full w-6 h-6 flex items-center justify-center text-xs">3</span>
                <span className="flex justify-center items-center gap-1">
                  <span>Documents</span>
                  <span className="bg-red-700 text-white rounded-full w-4 h-4 flex items-center justify-center text-xs">{student.total_pending_documents}</span>
                </span>
              </button>
            </li>
          </ul>
