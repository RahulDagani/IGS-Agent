                      ...
                      <div key={intake.intake_id} className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                        <div className={`p-3 ${isSelected ? 'bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-500' : ''}`}>
                          <div className="flex items-center justify-between">
                            ...
                            {/* Note: Since deadlines aren't in the intake data from the course API, 
                                we'll keep the expand/collapse but without deadline details */}
                            <button
                              type="button"
...
export default function StudentProgramsPage({ studentId }: ProgramsProps) {
  ...
  const fetchFilters = useCallback(async (currentModalFilters?: FilterOptions) => {
    try {
      ...
      const url = `${BASE_URL}/tenant/course/filters/dynamic${queryString ? `?${queryString}` : ''}`;
